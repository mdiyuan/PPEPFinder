...
def main(args):
    ...
    if args.model == "effectortransformer":
        ...
        model = ESM1bModel(1280, 33, unfreeze_last=True, hid_dim=args.hid_dim, dropout_rate=args.dropout_rate, num_classes=2)
    else:
    ...
    train_dataset = TXSESequenceDataSet(fasta_path=os.path.join(args.data_dir, 'train_output.fasta'),
                                transform=label2index, mode='train', kfold=args.kfold, fold_num=args.fold_num, seed=args.seed)
    valid_dataset = TXSESequenceDataSet(fasta_path=os.path.join(args.data_dir, 'train_output.fasta'),
                                transform=label2index, mode='valid', kfold=args.kfold, fold_num=args.fold_num, seed=args.seed)
    test_dataset = TXSESequenceDataSet(fasta_path=os.path.join(args.data_dir, 'test_output.fasta'),
    ...
    valid_best_loss, valid_best_metrics, valid_truth, valid_pred = test(model, valid_loader, criterion, device,  return_array=True, save_results=True, save_path='./valid_results.txt')
    _, test_final_metrics, test_truth, test_pred = test(model, test_loader, criterion, device,  return_array=True, save_results=True, save_path='./test_results.txt')
