...
def test(model, iterator, criterion, device, return_array=False, save_results=False, save_path=None):
            ...
            if save_results:
                with open(save_path, 'a') as file:
                    for i in range(len(labels)):
                        file.write(f"Sample {sample_index}: True label: {labels[i]}, Predicted label: {pred[i]}, Probabilities: {prob[i]}\n")
                        sample_index += 1  
