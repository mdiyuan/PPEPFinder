...
class EffectorTransformer(nn.Module):
    ...
    def __init__(self, emb_dim, repr_layer, num_layers, heads,
        ...
        super().__init__()
        self.pretrained_model, alphabet = esm.pretrained.esm1b_t33_650M_UR50S() #加载预训练的 ESM1b 模型
        # self.pretrained_model, alphabet = esm.pretrained.esm2_t33_650M_UR50D() #加载预训练的 ESM2 模型
...
class ESM1bModel(nn.Module):
    def __init__(self, emb_dim, repr_layer,
        ...
        super().__init__()
        self.pretrained_model, _ = esm.pretrained.esm1b_t33_650M_UR50S()#加载预训练的 ESM-1b 模型
    ...
    def forward(self, strs, toks):
        toks = toks[:, :1022] 
        batch = toks.shape[0]
        out = self.pretrained_model(toks, repr_layers=[self.repr_layer], return_contacts=False)  
